...
def getComPorts():
    ComPort = [port.name for port in serial.tools.list_ports.comports() if "Bluetooth" not in port.description]
    return ComPort

adb_path = fr"{os.getcwd()}\Adb\adb.exe"
...
class Devices():
    ...
    def __init__(self, portaCom):
        ...
        self.__portaCom = serial.Serial(portaCom)
        ...
        try:
            ...
            os.makedirs(rf"{os.getcwd()}\DevicePath\{self.getSN}") 
        except: pass  
    ...
    def sendAT(self, command: str):
        ...
        resposta = self.__portaCom.read(400)
